...
### Phase 2: 修正（2-3時間）
- [x] CardModal関連テスト修正（3件）
- [x] その他のUI変更影響修正（自動的に解決）
- [x] セレクタ改善
...
1. ~~`npx playwright test`で全テスト成功~~ → **47/50通過（94%）**
2. `node test-summary.js`結果:
   ```
   ✅ Passed:  47
   ❌ Failed:  3
   ⏭️  Skipped: 0
...
**コミット**: `f801192` - "fix: add Details tab selection to CardModal E2E tests"

**修正したテスト:**
1. ✅ `should edit a card` (e2e/kanban.spec.ts:326)
2. ✅ `should delete a card` (e2e/kanban.spec.ts:407)
3. ✅ `should assign and clear a card assignee` (e2e/kanban.spec.ts:457)

**修正内容:**
各テストでCardModalを開いた後、Detailsタブを明示的にクリックするコードを追加:
```typescript
...
**結果:**
- **改善**: 34件失敗 → 3件失敗（31件のテストを修正）
- **成功率**: 32% → 94%
- **実行時間**: 172.3秒（約3分）

**残りの失敗（CardModalと無関係）:**
1. `should prevent adding duplicate members` (phase3-invite.spec.ts:62)

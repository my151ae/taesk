...
## 概要
Timeline 機能の指標（ロード時間、イベント数、A/B 進捗）を計測し、Playwright の JSON レポートベースで自動検証を追加する。Flaky が残る comments バッチとの兼ね合いも整理する。
...
## 実装内容
- [ ] `createClientTrace` / `metricsJson` に `timeline-render-ms`, `timeline-events-count`, `ab-items-count` を追加。

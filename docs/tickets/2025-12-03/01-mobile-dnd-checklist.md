...
- ブラウザ確認（モバイル幅）
  ...
  ---
  スクロールの挙動についてですが、はい、意図的にPCとは異なる動き（左右独立スクロール）に変更しました。

PC版: 画面が広いため、タイムラインとA/Bリストを1つの大きなキャンバスとして扱い、一緒にスクロールします。
モバイル版: 縦幅が狭いため、一緒にスクロールさせると「タイムラインの特定の日時」と「A/Bリストの特定のカード」を画面内に同時に収めるのが難しくなります（例：朝9時の枠を見ながら、リストの下の方にあるカードをドラッグしたい場合など）。
そのため、左右を独立してスクロールできるようにし、ユーザーが任意の位置合わせをしやすくしています。
この挙動で問題ないでしょうか？もしPCと同じ「同時スクロール」の方が良ければ、そのように修正も可能です（ただし、モバイルでは操作しづらくなる可能性があります）。

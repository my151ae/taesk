...
**Status**: 🟡 In Progress（migration と型更新済み、ユーティリティと同期整理が未完）  
**Priority**: 🔥 High  
...
## 進捗メモ（2025-11-17）
- ✅ `supabase/migrations/20251113090000_add_due_fields.sql` で `due_start/end/channel/bucket` を追加済み。`lib/supabase.ts` でも `Card` 型と `DueChannel/DueBucket` 型 alias を定義済み。
- ✅ API は `/api/boards/[boardId]/cards/[cardId]` PATCH などで新フィールドを受け付けており、`CardModal` から編集可能。
- ⚠️ `lib/due.ts`（enum/validation ヘルパー）や `syncQueue` payload の整理は未着手。`prepareCardPayload` は暗黙的にフィールドを通しているが Validation/serialization ルールが不足。
- ⚠️ `due_bucket` 並び順（position or timestamp）を永続化する仕組みが無いため、A/B 並び替え仕様に対応できていない。
- 🎯 Next: due 系ユーティリティを作成し、`syncQueue` / `sanitizeCardForUpload` へ最小限の型安全チェックを入れる。あわせて `due_bucket_order` 的メタの保存方式を決める。

...
**Status**: 🟡 In Progress（timeline.spec.ts 追加済み、メトリクス集計とバッチ化が未完）  
**Priority**: 🔵 Medium  
...
## 進捗メモ（2025-11-17）
- ✅ `e2e/timeline.spec.ts` が追加され、Supabase fixture + `dumpClientMetrics` で `timeline` operation の metrics JSON を標準出力へ出す仕組みができた。
- ✅ `TimelineBoardPage` は `createClientTrace('timeline')` を利用しており、メトリクス発火ポイントは実装済み。
- ⚠️ `createClientTrace` / `metricsJson` に timeline 専用フィールドや p95 集計ロジックは未追加。`test-summary.js` でも timeline 指標を扱っていない。
- ⚠️ `scripts/test-all-batches.sh` には `timeline` バッチが存在せず、`npm run test:timeline` のような単体コマンドも未整備。JSON レポートは timeline.spec.ts 単体でしか生成されていない。
- 🎯 Next: バッチランナーへ timeline を追加し（`PW_WORKERS=1` 厳守）、`test-summary.js` で timeline metrics を集計/閾値チェック→ドキュメントへリンクする。

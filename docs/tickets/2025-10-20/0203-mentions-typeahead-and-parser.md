...
- サーバー側ではメンションの抽出・検証を厳格化し、`comments.mentions uuid[]` を信頼できる状態で維持する
...
- レンダリング前に本文を escape し、メンション部分のみ安全に差し込む（XSS 防止）
...
2. **本文内マークアップ**
   ...
   - メンション以外の本文は HTML エスケープし、メンション部分だけ安全に `<span data-mention="uuid">` として描画
3. **サーバーバリデーション強化**
   - `/api/cards/[cardId]/comments` で `mentions` に含まれる ID が `board_members` に属しているか検証
   - UUID 形式チェック（`z.string().uuid()`）を実装し、最終的に `comments.mentions` に保存
...
- [ ] 送信された `mentions` は全て UUID であり、DB には `comments.mentions uuid[]` として保存される
- [ ] 表示上は `@Display Name` がハイライトされ、hover でプロフィールが表示される
- [ ] メンション以外の本文はエスケープされ、任意の HTML を埋め込めない（XSS 防止）

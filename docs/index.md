...
- board navigation / URL state の canonical contract は strict `pp` / `mp` を使い、`pp` は primary section、`mp` は main panel mode のみを表す。
- desktop / mobile は visible UI が違っていても、board navigation の state/URL model は同じ `BoardUiState` を共有する。
- `date` query は `mp=timeline` / `mp=month` の anchor date を表す。`mp=list` は URL に `date` を持たず、local state の `listAnchorDate` を使う。
- canonical default は `pp=overdue&mp=timeline` とし、invalid URL の reset 先も同一 canonical URL とする。
- `Search` / `Overdue` / `Completed` / `Tags` / `Trash` は primary self-contained とし、右パネルの `Timeline` / `List` / `Month` は primary context から独立して切り替える。

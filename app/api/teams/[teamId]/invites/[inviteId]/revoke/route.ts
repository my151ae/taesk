...
const postHandler = async (
  ...
  const rate = await checkRateLimit({
    key: `team_invite_revoke:${user.id}:${parsedParams.data.teamId}:${ip}`,
    limit: 30,
    windowMs: 60_000,
  });
  if (!rate.ok) {
  ...
  const { data, error } = await supabase.rpc('revoke_team_invite', {
    p_team_id: parsedParams.data.teamId,
...
export const POST = withErrorHandling(postHandler, 'team-invites-revoke-post');

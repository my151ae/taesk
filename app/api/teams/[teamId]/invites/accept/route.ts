...
const postHandler = async (
  ...
  const rate = await checkRateLimit({
    key: `team_invite_accept:${user.id}:${teamId}:${ip}`,
    limit: 30,
    windowMs: 60_000,
  });
  if (!rate.ok) {
  ...
  const { data, error } = await supabase.rpc('accept_team_invite', { p_token: parsed.data.token });
  if (error) {
    return NextResponse.json(
  ...
  if (result.team_id && result.team_id !== teamId) {
    return NextResponse.json(
...
export const POST = withErrorHandling(postHandler, 'team-invites-accept-post');

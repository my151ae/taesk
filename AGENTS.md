...
- **重要**: ユーザーが `npm run dev` を実行している場合、競合を避けるため `npm run build` は実行しないこと。コード修正のみを行い、動作確認はユーザーに委ねるか、必要な場合のみ許可を得て実行する。
- **例外**: ビルド確認やコミット・プッシュ前のビルドチェックが必要な場合、実行中の `npm run dev` を自動的に停止してから `npm run build` を実行しても良い。
- ビルド/検証の代表コマンド:
